...
import 'package:backtest/src/model/account/portfolio.dart';
...
class TradingAccount with Portfolio {
  ...
  void openTrade(String ticker, TradeType tradeType, DateTime date, double price) {
  ...
  void closeTrade(String ticker, TradeType tradeType, DateTime date, double close) {
    if (isTradeOpen(ticker, tradeType)) {
      ...
      if (tradeOpen != null) {
        tradesHistory.add(
            TradeHistory(trade: tradeOpen, closeDate: date, closePrice: close, maxDrawdown: tradeOpen.maxDrawdown));
      }
    }
  }

  void updateAccount(YahooFinanceCandleData currentCandle, YahooFinanceCandleData? previousCandle) {
    // If nothing changed
