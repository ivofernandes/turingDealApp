...
class _PortfolioWidgetState extends State<PortfolioWidget> {
                ...
                return Column(
                  children: [
                      ...
                      children: [
                        ...
                        Expanded(
                          child: TextFormField(
                            ...
                            initialValue:
                                tickerAllocation.numberOfShares.toString(),
                            onChanged: (value) {
                    ...
                    Slider(
                      ...
                      onChanged: tickerAllocation.isSelected
                          ? (value) => updateSliderPercentage(ticker, value)
                          : null,
                      min: 0,
                      max: 100,
                    ),
                    Text(
                        '${tickerAllocation.percentageOfPortfolio.toStringAsFixed(2)}%'),
                  ],
                );
              }).toList(),
            ),
  ...
  void updateSliderPercentage(String ticker, double value) {
    ...
    for (var ticker in allocations.keys) {
      allocations[ticker]!.percentageOfPortfolio =
          allocations[ticker]!.percentageOfPortfolio / totalPercentage * 100;
    }
  ...
  void updateNumberOfShares() {
    ...
    for (final ticker in allocations.keys) {
      totalAmount +=
          allocations[ticker]!.numberOfShares * allocations[ticker]!.endPrice;
    }
    ...
    for (final ticker in allocations.keys) {
      ...
      if (allocations[ticker]!.endPrice != 0) {
        numberOfShares =
            (percentage / 100 * totalAmount / allocations[ticker]!.endPrice)
                .round();
      }
  ...
  void updateAllocationsPercentages() {
    ...
    for (final ticker in allocations.keys) {
      final tickerAllocation = allocations[ticker]!;
      final tickerAmount =
          tickerAllocation.numberOfShares * tickerAllocation.endPrice;
      ...
      debugPrint(
          '$ticker | percentageOfPortfolio: ${tickerAllocation.percentageOfPortfolio}');
    }
