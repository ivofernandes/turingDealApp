...
import 'package:backtest/src/model/strategy_result/strategy_drawdown.dart';
...
class CalculateDrawdown {
  ...
  static StrategyDrawdown calculateStrategyDrawdown(List<YahooFinanceCandleData> prices) {
    ...
    return StrategyDrawdown(currentDrawdown: currentDrawdown, maxDrawdown: maxDrawdown);
  }

  static void updateTradeDrawdown(TradeOpen position, YahooFinanceCandleData currentCandle) {
    double currentDrawdown = 0;

    if (position.tradeType == TradeType.LONG) {
      currentDrawdown = calculatePercentageChange(position.tradeType, position.mostFavorablePrice, currentCandle.low);
    } else if (position.tradeType == TradeType.SHORT) {
      currentDrawdown = calculatePercentageChange(position.tradeType, position.mostFavorablePrice, currentCandle.high);
    }
  ...
  static double calculatePercentageChange(TradeType type, double entry, double exit) {
